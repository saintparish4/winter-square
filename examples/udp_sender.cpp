...
#ifdef _WIN32
// Windows stub - this example only runs on Linux/WSL
int main() {
  std::cerr << "This example requires Linux/WSL - build and run in WSL\n";
  return 1;
}
#else
#include <arpa/inet.h>
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    char buffer[1024];
    int len = snprintf(
        buffer, sizeof(buffer), "TEST_PACKET seq=%" PRIu64 " timestamp=%lld",
        seq++,
