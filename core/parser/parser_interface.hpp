...
public:
  ...
  virtual size_t parse(const MessageView &raw_packet, NormalizedMessage *output,
                       size_t max_messages) noexcept = 0;
  ...
  virtual void get_stats(Statistics &stats) const { (void)stats; }
};

// Null parser - passes through without parsing (for testing)
class NullParser : public IParser {
public:
  size_t parse(const MessageView &raw_packet, NormalizedMessage *output,
  ...
  const char *name() const noexcept override { return "NullParser"; }
};
...
public:
  size_t parse(const MessageView &raw_packet, NormalizedMessage *output,
               size_t max_messages) noexcept override {
    if (max_messages == 0 || !raw_packet.is_valid())
      return 0;
  ...
  const char *name() const noexcept override { return "EchoParser"; }
